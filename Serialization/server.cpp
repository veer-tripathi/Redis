...
static void out_dbl(Buffer &out, double n){
    buf_append_u8(out, TAG_DBL);
    buf_append_dbl(out,n);
}

static void out_err(Buffer& out , uint32_t code, const std:: string& s){
    buf_append_u8(out, TAG_ERR);
    buf_append_u32(out, code);
    buf_append_u32(out, s.size());
...
static bool eq(Hnode* l, Hnode* r){
    struct Entry* le=container_of(l,struct Entry, node);
    struct Entry* re=container_of(r,struct Entry, node);
...
static void do_del(std::vector<std::string> &cmd, Buffer& out) {
    ...
    key.node.hcode = str_hash((uint8_t *)key.key.data(), key.key.size());
    // hashtable delete
    Hnode *node = hm_delete(&g_data.db, &key.node, &eq);
